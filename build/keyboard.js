...
const menuKeyboard = id => {
    ...
    keyboard.push([_utils.Markup.callbackButton('User', `user/${id}`),
    // Coundown has all because when it's available the user could select between animes or mangas
    _utils.Markup.callbackButton('Countdown', `countdown/${id}/all`), _utils.Markup.callbackButton('Guide', `guide/${id}`)]);

    keyboard.push([_utils.Markup.callbackButton('Watchlist', `watchlist/${id}/all`), _utils.Markup.callbackButton('Readlist', `readlist/${id}/all`)]);
...
const timeKeyboard = (period, tz) => {
    ...
    keyboard.push([_utils.Markup.callbackButton('1h', `time/${1 + add}/${tz}`), _utils.Markup.callbackButton('2h', `time/${2 + add}/${tz}`), _utils.Markup.callbackButton('3h', `time/${3 + add}/${tz}`), _utils.Markup.callbackButton('4h', `time/${4 + add}/${tz}`)]);

    keyboard.push([_utils.Markup.callbackButton('5h', `time/${5 + add}/${tz}`), _utils.Markup.callbackButton('6h', `time/${6 + add}/${tz}`), _utils.Markup.callbackButton('7h', `time/${7 + add}/${tz}`), _utils.Markup.callbackButton('8h', `time/${8 + add}/${tz}`)]);

    keyboard.push([_utils.Markup.callbackButton('9h', `time/${9 + add}/${tz}`), _utils.Markup.callbackButton('10h', `time/${10 + add}/${tz}`), _utils.Markup.callbackButton('11h', `time/${11 + add}/${tz}`), _utils.Markup.callbackButton('12h', `time/${12 + add}/${tz}`)]);
