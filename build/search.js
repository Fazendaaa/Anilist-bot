 ...
 */
const getSearch = (value, _ref) => {
  let type = _ref.type,
      reply = _ref.reply,
      keyboard = _ref.keyboard;
  return new Promise((resolve, reject) => {
    _nani2.default.get(`${type}/search/${value}`).then(data => {
      if (!data.hasOwnProperty('error')) resolve({
 ...
 */
const animeSearch = anime => getSearch(anime, { type: 'anime', reply: _reply.replyAnime, keyboard: _keyboard.animeKeyboardSearch });
 ...
 * @returns {string} Fetched data in Telegram standards.
 */
const characterSearch = character => getSearch(character, { type: 'character', reply: _reply.replyCharacter, keyboard: _keyboard.characterKeyboard });
 ...
 * @returns {string} Fetched data in Telegram standards.
 */
const staffSearch = staff => getSearch(staff, { type: 'staff', reply: _reply.replyStaff, keyboard: _keyboard.staffKeyboard });
 ...
 * @returns {string} Fetched data in Telegram standards.
 */
const studioSearch = studio => getSearch(studio, { type: 'studio', reply: _reply.replyStudio });
 ...
 * @returns {string} Fetched data in Telegram standards.
 */
const mangaSearch = manga => getSearch(manga, { type: 'manga', reply: _reply.replyManga, keyboard: _keyboard.mangaKeyboardSearch });
...
const searchAll = (type, value) => new Promise((resolve, reject) => {
  _nani2.default.get(`${type}/search/${value}`).then(data => {
    if (!data.hasOwnProperty('error')) resolve(data);else reject('error');
  }).catch(error => {
    error.description = error;
    reject(error);
  });
...
const page = (type, id) => new Promise((resolve, reject) => {
  _nani2.default.get(`${type}/${id}/page`).then(data => {
    if (!data.hasOwnProperty('error')) resolve(data);else resolve('error');
  }).catch(error => {
...
const getID = (type, id) => new Promise((resolve, reject) => {
  _nani2.default.get(`${type}/${id}`).then(data => {
    if (!data.hasOwnProperty('error')) resolve(data);else resolve('error');
  }).catch(error => {
...
const getBrowse = type => new Promise((resolve, reject) => {
  _nani2.default.get(`browse/${type}`).then(data => {
    if (!data.hasOwnProperty('error')) return data;else resolve('error');
  }).then(data => data.map(element => (0, _reply.replyBrowse)(type, element))).then(resolve).catch(error => {
...
const inlineSearchByType = (type, search, browseFunc, funcSearch) => {
  if ('' == search) return browseFunc();else return funcSearch(search).then(data => !data.hasOwnProperty('error') ? data.map(element => (0, _reply.replyInline)(type, element)) : []);
};
...
const inlineSearchAll = search => {
  ...
  return Promise.all(types.map(type => _nani2.default.get(`${type}/search/${search}`).then(data => !data.hasOwnProperty('error') ? data.map(element => (0, _reply.replyInline)(type, element)) : [])));
};
...
const inlineSearch = query => new Promise((resolve, reject) => {
  ...
  if ('' == query) search = Promise.resolve([_utils.searchMessage]);else {
    ...
    if (/\bmanga\b/.exec(query)) search = inlineSearchManga(query);else if (query.includes('anime')) search = inlineSearchAnime(query);else if (query.includes('character')) search = inlineSearchCharacter(query);else if (query.includes('staff')) search = inlineSearchStaff(query);else if (query.includes('studio')) search = inlineSearchStudio(query);
...
module.exports = {
  ...
  searchAnimeRelaseDate: searchAnimeRelaseDate
};